...
def main():
	...
	error_implicit_krasulina = np.zeros((M+1, len(err_iters)+1, num_iter))
	...
	eta0_implicit_krasulina = tune_params_implicit_krasulina(Y_valid, k, eta_vals)
	for itr in range(num_iter):
		...
		error_implicit_krasulina[:,err_idx,itr] = np.mean(np.sum((Y - dot(C_init,X))**2, axis=0))
		err_idx += 1

		eta0 = eta0_implicit_krasulina
		a = 0.8
		for nn in range(num_example_per_machine):
			...
			if (nn+1) in err_iters:
				...
				error_implicit_krasulina[0,err_idx,itr] = np.mean(np.sum((Y - dot(C,X))**2, axis=0))
				...
				for mm in range(1, M):
					...
					Cp = Cp_all[mm]
					X = dot(Cp, np.dot(C.T, Y))
					error_implicit_krasulina[mm,err_idx,itr] = np.mean(np.sum((Y - dot(C,X))**2, axis=0))
					CT = CT + C
				C = CT/M
				Cp = inv(dot(C.T, C))
				...
				X = dot(Cp, np.dot(C.T, Y))
				error_implicit_krasulina[M,err_idx,itr] = np.mean(np.sum((Y - dot(C,X))**2, axis=0))
				err_idx += 1
				print("Implicit krasulina iter %d step %d/%d, k = %d, mean error %f, combined error %f" % (itr+1, nn+1, num_example_per_machine, k, np.mean(error_implicit_krasulina[:M,err_idx-1,itr]), error_implicit_krasulina[M,err_idx-1,itr]))

		sio.savemat('../results/distributed_pca_implicit_krasulina_' +  filename + '_k_' + str(k) + '.mat', {'error_imp_krasulina':error_implicit_krasulina})
